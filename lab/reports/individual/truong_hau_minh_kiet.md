...
Tôi cũng quyết định dùng thêm grading JSONL làm lớp kiểm cuối. Chỉ nhìn `eval_retrieval.py` là chưa đủ vì grading còn yêu cầu `gq_d10_03` phải đúng cả `contains_expected`, `hits_forbidden=false` và `top1_doc_matches=true`. Sau khi cập nhật bộ câu hỏi ở thư mục gốc, tôi rerun bằng `python grading_run.py --questions grading_questions.json --out artifacts/eval/grading_run_from_root.jsonl` rồi kiểm tra lại với `python instructor_quick_check.py --grading artifacts/eval/grading_run_from_root.jsonl`. Nhờ vậy tôi có thể chốt trạng thái index cuối bằng artifact mới thay vì chỉ đọc tay CSV.
...
Tôi phát hiện vấn đề bằng `artifacts/logs/run_inject-bad.log`: dòng `expectation[refund_no_stale_14d_window] FAIL (halt) :: violations=1` cho thấy stale refund đã lọt vào cleaned set khi cố ý bỏ refund fix. Sau đó tôi rerun luồng chuẩn bằng `final-submit`, sinh lại `after_fix_final-submit.csv`, rồi kiểm tra thấy câu `q_refund_window` có `contains_expected=yes` và `hits_forbidden=no`. Cuối cùng tôi rerun grading bằng `grading_questions.json` ở thư mục gốc để tạo `artifacts/eval/grading_run_from_root.jsonl`; cả ba câu `gq_d10_01..03` đều `OK`. Tôi xem đây là anomaly quan trọng nhất vì nó chứng minh data bug có thể không đổi volume nhưng vẫn làm agent đọc nhầm context.
...
Tôi cũng kiểm lại câu Merit là `q_leave_version`; ở `artifacts/eval/grading_run_from_root.jsonl`, `gq_d10_03` có `contains_expected=true`, `hits_forbidden=false`, `top1_doc_matches=true`, còn `gq_d10_01` và `gq_d10_02` cũng tiếp tục đúng theo rubric. Điều này giúp tôi yên tâm rằng khi sửa refund pipeline không làm hỏng policy HR 2026.
