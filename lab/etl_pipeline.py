...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    # Sprint 1 Bonus (+1): timestamp tại ranh giới Ingest
    # Ghi ngay khi đọc file thành công — trước mọi transform.
    ...
    # EMBED PHASE
    # -----------------------------------------------------------------------
    embed_ok, publish_ts = cmd_embed_internal(
        cleaned_path,
        run_id=run_id,
        log=log,
    )
    ...
    # Sprint 2 Bonus (+1): timestamp tại ranh giới Publish
    # publish_ts rỗng nếu cleaned CSV rỗng (không có gì để embed).
    if publish_ts:
        log(f"publish_boundary_ts={publish_ts}")
    ...
    manifest = {
        ...
        # Sprint 1 Bonus: ranh giới Ingest
        "ingest_boundary_ts": ingest_ts,
        # Sprint 2 Bonus: ranh giới Publish (rỗng nếu CSV trống)
        "publish_boundary_ts": publish_ts,
...
def cmd_embed_internal(
    ...
    *,
    ...
    Trả về (success: bool, publish_ts: str).
    publish_ts là ISO timestamp ngay sau col.upsert() thành công.
    publish_ts = "" nếu embed không thực hiện (CSV rỗng) hoặc thất bại.
    """
    try:
        import chromadb
